...
export const register = async(req, res) => {
    ...
    try {
       ...
       const savedUser = await newUser.save(); //gurado el nuevo usuario en la base de datos
       ...
       await transport.sendMail({
        ...
        context:{
            ...
            verificationLink //aca le paso las variables para que las use en la plantilla despues
        }
       });
       ...
       // 3 - Responder al cliente con la cookie(es el dato que necesita el usuario para persistir la sesion)
...
export const login = async(req,res) => {
    try {
        ...
        const userFound = await User.findOne({email}); // busco si el usuario existe en mongo
...
export const verifyEmail = async (req,res) => {
    try {
     ...
     const user = await User.findOne({verificationToken:token}); //compruebo de que exista el mismo token
     ...
     await user.save(); //actualizo la base de datos
