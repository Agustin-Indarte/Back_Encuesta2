...
export const requestPasswordReset = async (req,res) => {
    try {
        ...
        user.passwordResetExpires = new Date(Date.now() + 600000) // 10 minutos
        await user.save()

        // 5 - Enviar email con el link de reset
        const resetLink = `${process.env.FRONTEND_URL}/reset-password?token=${resetToken}`; //direccion del fronmt que va a recibir el token
...
export const resetPassword = async (req,res) => {
    try {
        ...
        // 2 - Buscar al usuario, buscamos por los tres parametros
